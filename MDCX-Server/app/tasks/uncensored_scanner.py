...
class UncensoredScanner(BaseScanner):
    ...
    async def scan(self) -> dict:
        ...
        logger.info(f"[uncensored] 扫描启动: media_dirs={[str(d) for d in self.media_dirs]}")
        for media_dir in self.media_dirs:
            try:
                logger.info(f"[uncensored] 开始扫描目录: {media_dir}")
                dir_result = await self._scan_directory(media_dir)
                logger.info(
                    f"[uncensored] 目录扫描完成: {media_dir} 共发现 {dir_result['total']} 个文件，"
        ...
        logger.info(
            f"[uncensored] 扫描完成: 共发现 {results['total']} 个文件，新增 {results['movies_added']}，"
