...
@register_crawler
class MGStageCrawler(LegacyCrawlerAdapter):
    """MGStage 爬虫"""

    name = "mgstage"
    display_name = "MGStage"
    base_url = "https://www.mgstage.com"

    priority = CrawlerPriority.HIGH
    supported_types = ['jav']
    supported_prefixes = []
    description = "MGStage 日文源"
    language = "ja"
    ...
    async def _call_mdcx_main(self, code: str, client: AsyncHttpClient) -> Optional[dict]:
        ...
        try:
            ...
            for real_url in real_url_list:
                htmlcode = await client.get_text(real_url)
                if not htmlcode:
                    continue

                htmlcode_parsed = etree.fromstring(htmlcode, etree.HTMLParser())
                actor = getActor(htmlcode_parsed).replace(" ", "").strip(",")
                title = getTitle(htmlcode_parsed).replace("\\n", "").replace("        ", "").strip(",").strip()
                if title:
                    break
            else:
                return None

            cover_url = getCover(htmlcode_parsed)
            poster_url = getCoverSmall(cover_url)
            outline = getOutline(htmlcode_parsed).replace("\n", "").strip(",")
            release = getRelease(htmlcode_parsed).strip(",").replace("/", "-")
            tag = getTag(htmlcode_parsed).strip(",")
            year = getYear(release).strip(",")
            runtime = getRuntime(htmlcode_parsed).strip(",")
            score = getScore(htmlcode_parsed).strip(",")
            series = getSeries(htmlcode_parsed).strip(",")
            studio = getStudio(htmlcode_parsed).strip(",")
            publisher = getPublisher(htmlcode_parsed).strip(",")
            actor_photo = getActorPhoto(actor.split(","))
            extrafanart = getExtraFanart(htmlcode_parsed)
            ...
            return {"mgstage": {"zh_cn": dic, "zh_tw": dic, "jp": dic}}

        except Exception as e:
            logger.error(f"MGStage scrape error for {code}: {e}")
            return None
