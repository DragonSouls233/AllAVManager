...
async def main(
    ...
    **kwargs,
    ...
    use_proxy = True  # JavLibrary 通常需要代理
...
@register_crawler
class JavLibraryCrawler(LegacyCrawlerAdapter):
    """JavLibrary 爬虫"""

    name = "javlibrary"
    display_name = "JavLibrary"
    ...
    async def _call_mdcx_main(self, code: str, client: AsyncHttpClient) -> Optional[dict]:
        ...
        try:
            ...
            url_search = javlibrary_url + code
            html_search = await client.get_text(url_search)
            if not html_search:
                return None

            html = etree.fromstring(html_search, etree.HTMLParser())
            real_url = get_real_url(html, code, domain_2)
            if not real_url:
                return None

            html_info = await client.get_text(real_url)
            if not html_info:
                return None
            ...
            web_number = get_number(html_detail, code)
            title = title.replace(web_number + " ", "")
            actor = get_actor_str(html_detail)
            actor_photo = get_actor_photo(actor)
            ...
            return {"javlibrary": {"zh_cn": dic}}

        except Exception as e:
            logger.error(f"JavLibrary scrape error for {code}: {e}")
            return None
