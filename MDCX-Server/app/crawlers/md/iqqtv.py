...
@register_crawler
class IQQTVCrawler(LegacyCrawlerAdapter):
    """IQQTV 爬虫"""

    name = "iqqtv"
    display_name = "IQQTV"
    base_url = "https://iqqtv.cloud"

    priority = CrawlerPriority.NORMAL
    supported_types = ['jav']
    supported_prefixes = []
    description = "IQQTV 多语言源"
    language = "zh"
    ...
    async def _call_mdcx_main(self, code: str, client: AsyncHttpClient) -> Optional[dict]:
        ...
        language = _normalize_language(language)
        appoint_url = appoint_url.replace("/cn/", "/jp/").replace("iqqtv.cloud/player", "iqqtv.cloud/jp/player")
        json_data = await _main_single_language(number, appoint_url, "jp", client=client)
        ...
        json_data_zh = await _main_single_language(number, appoint_url, language, client=client)
        dic = json_data_zh["iqqtv"][language]
        dic["originaltitle"] = json_data["iqqtv"]["jp"]["originaltitle"]
        dic["originalplot"] = json_data["iqqtv"]["jp"]["originalplot"]
        json_data["iqqtv"].update({language: dic})
