...
def _get_mod_cls(module: str, cls_name: str):
    """获取模块中的任意模型类（如 MovieActor, Studio, Series, Tag, MovieTag 等）"""
...
async def list_movies(
    ...
    ActorModel = _get_mod_model(module, "actor")
    ...
    cache_key = (
        f"movies:list:{module}:{page}:{page_size}:{status}:{maker}:{studio}:{series}:{tag_id}:{actor_id}"
    ...
    if status:
        query = query.where(MovieModel.status == status)
    ...
    if tag_id_list:
        if tag_mode_upper == "AND":
            ...
            query = query.join(MovieTag, MovieModel.id == MovieTag.movie_id).where(
                MovieTag.tag_id.in_(tag_id_list)
            ).group_by(MovieModel.id).having(
    ...
    if actor_id:
        query = query.join(MovieActor, MovieModel.id == MovieActor.movie_id).where(MovieActor.actor_id == actor_id)

    # 收藏过滤（从 system.db 查收藏的 movie ID，再过滤模块查询）
    if is_favorite is True or favorite_group_id is not None:
        ...
        fav_result = await sys_session.execute(fav_subq)
        fav_ids = [row[0] for row in fav_result.fetchall()]
        if fav_ids:
            query = query.where(MovieModel.id.in_(fav_ids))
        else:
            # 没有收藏数据，直接返回空
            return MovieListResponse(total=0, items=[])
    elif is_favorite is False:
        fav_result = await sys_session.execute(
    ...
    if min_rating is not None:
        query = query.where(MovieModel.rating >= min_rating)
    if max_rating is not None:
        query = query.where(MovieModel.rating <= max_rating)
    ...
    if letter:
        ...
        if norm == "#":
            # 数字开头
            first = func.upper(func.substr(MovieModel.code, 1, 1))
            query = query.where(first.between("0", "9"))
        elif len(norm) == 1 and norm.isalpha():
            query = query.where(func.upper(func.substr(MovieModel.code, 1, 1)) == norm)
    ...
    if seed is not None:
        ...
        query = query.order_by((MovieModel.id * seed) % 9973, MovieModel.id)
    elif sort:
        sort_map = {
            "release_date": MovieModel.release_date,
            "duration": MovieModel.duration,
            "play_count": MovieModel.play_count,
    ...
    if movie_ids:
        actor_query = (
            select(MovieActor.movie_id, ActorModel.id, ActorModel.name)
            .join(ActorModel, ActorModel.id == MovieActor.actor_id)
...
async def get_alphabet_groups(
    ...
    cache_key = f"movies:alphabet:{module}:{status}"
    cached = _cache.get(cache_key, ttl=120)
    if cached is not None:
        return cached

    first_char = func.upper(func.substr(MovieModel.code, 1, 1))
    query = select(
        first_char.label("letter"),
        func.count(MovieModel.id).label("cnt"),
    ).group_by(first_char)
...
async def _backfill_cover(movie, path_str: str, session) -> None:
...
async def _resolve_cover_path(movie, t_param: Optional[str], session) -> Optional[str]:
...
def _web_cover_redirect(movie) -> Optional["RedirectResponse"]:
    """封面是相对 web 路径(如 /pics/cover/)或远程 URL 时，直接 302 到该地址"""
...
async def run_cover_backfill(session, limit: int = 2000, module: str = "jav") -> dict:
...
async def backfill_covers(
    ...
    session = await get_module_session(module)
    return await run_cover_backfill(session, limit, module)
...
async def backfill_file_paths(
    ...
    rows = await session.execute(
        select(MovieModel).where(
            or_(MovieModel.file_path.is_(None), MovieModel.file_path == ""),
...
async def scan_and_link(
    ...
    # 2. 获取数据库中所有未关联路径的影片
    query = select(MovieModel).where(MovieModel.file_path.is_(None))
    result = await session.execute(query)
...
async def auto_link_files(
    ...
    for media_dir in media_dirs:
        ...
        for video_file in dir_path.rglob("*"):
            ...
            result = await session.execute(
                select(MovieModel).where(MovieModel.code == normalized)
            )
...
async def get_related_movies(
    ...
    if actor_id_list:
        result = await session.execute(
            select(MovieModel)
            .join(MovieActor, MovieModel.id == MovieActor.movie_id)
            .where(
    ...
    if movie.series_id:
        result = await session.execute(
            select(MovieModel)
            .where(MovieModel.series_id == movie.series_id, MovieModel.id != movie_id)
            .order_by(MovieModel.release_date.desc())
            .limit(limit)
        )
        for m in result.scalars().all():
    ...
    if genre_list:
        ...
        genre_conditions = [MovieModel.genre.like(f"%{g}%") for g in genre_list[:5]]
        result = await session.execute(
            select(MovieModel)
            .where(
                or_(*genre_conditions),
                MovieModel.id != movie_id,
                ~MovieModel.id.in_(related_ids),
            )
            .order_by(MovieModel.release_date.desc())
            .limit(limit)
        )
...
async def update_movie(
    ...
    if "code" in body and body["code"] is not None and body["code"] != movie.code:
        ...
        existing = await session.scalar(select(MovieModel).where(MovieModel.code == new_code, MovieModel.id != movie_id))
        if existing:
    ...
    if "actors" in body and body["actors"] is not None:
        ...
        for nm in raw_actors:
            nm = str(nm).strip()
            if not nm: continue
            actor = await session.scalar(select(ActorModel).where(ActorModel.name == nm))
            if not actor:
                actor = ActorModel(name=nm, movie_count=0)
                session.add(actor)
                await session.flush()
    ...
    if sync_nfo:
        try:
            mv = await session.scalar(
                ...
                .options(selectinload(MovieModel.studio_ref), selectinload(MovieModel.series_ref))
            )
...
async def batch_delete_movies(
    ...
    stmt = sa_delete(MovieModel).where(MovieModel.id.in_(ids))
    ...
    if actor_names:
        ...
        for actor_name in actor_names:
            ...
            actor_obj = await session.scalar(
                select(ActorModel).where(ActorModel.name == actor_name.strip())
            )
            if not actor_obj:
                actor_obj = ActorModel(name=actor_name.strip())
...
async def scrape_by_code(
    ...
    movie = await session.scalar(select(MovieModel).where(MovieModel.code == code))
    if movie is None and norm != code:
        movie = await session.scalar(select(MovieModel).where(MovieModel.code == norm))

    if not force and movie is not None:
        nfo_result = await _try_scrape_from_nfo(movie)
        if nfo_result:
            resp = await _apply_scrape_result(session, movie, nfo_result, module)
            if resp["status"] == "ok":
                await session.commit()
                await session.refresh(movie)
                _cache.invalidate("movies:")
    ...
    resp = await _apply_scrape_result(session, movie, result, module)
    if resp["status"] == "ok":
        await session.commit()
        await session.refresh(movie)
        _cache.invalidate("movies:")
...
async def get_movie(
    ...
    cache_key = f"movies:detail:{module}:{movie_id}"
