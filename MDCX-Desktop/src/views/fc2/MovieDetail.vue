<template>
  <div class="movie-detail" v-loading="loading">
    <div class="detail-header">
      <el-button text @click="goBack"><el-icon><ArrowLeft /></el-icon> 返回FC2列表</el-button>
...
import { getFc2Movie, updateFc2Movie } from '@/api/fc2'
import { scrapeFc2Movie, reloadFc2MovieNfo } from '@/api/fc2'
...
function goBack() { router.push('/fc2') }
function play() {
  if (!canPlay.value) { ElMessage.warning('该影片没有关联视频文件'); return }
  router.push({ path: `/play/${route.params.id}`, query: { module: 'fc2' } })
}
