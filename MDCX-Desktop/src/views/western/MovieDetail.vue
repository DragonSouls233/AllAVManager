<template>
  <div class="movie-detail" v-loading="loading">
    <div class="detail-header">
      <el-button text @click="goBack"><el-icon><ArrowLeft /></el-icon> 返回欧美列表</el-button>
...
import { getWesternMovie, scrapeWesternMovie, reloadWesternMovieNfo } from '@/api/western'
...
function goBack() { router.push('/western') }
function play() {
  if (!canPlay.value) { ElMessage.warning('该影片没有关联视频文件'); return }
  router.push({ path: `/play/${route.params.id}`, query: { module: 'western' } })
}
