<template>
  <div class="western-movies">
    <div class="toolbar">
      <el-input v-model="keyword" placeholder="搜索标题/code..." clearable style="width: 280px">
        <template #prefix><el-icon><Search /></el-icon></template>
      </el-input>
      <el-select v-model="siteFilter" placeholder="品牌" clearable style="width: 180px">
        <el-option v-for="s in siteOptions" :key="s" :label="s" :value="s" />
      </el-select>
      <el-select v-model="networkFilter" placeholder="网络" clearable style="width: 180px">
      ...
      <div v-for="m in store.movies" :key="m.id" class="movie-card" @click="goDetail(m.id)">
        ...
        <div class="info">
          ...
          <div class="meta">
            ...
            <span v-if="m.release_date" class="date">{{ m.release_date }}</span>
          </div>
          <div class="tags" v-if="m.studio">
            <el-tag size="mini" type="info">{{ m.studio }}</el-tag>
          </div>
        </div>
      </div>
      <el-empty v-if="!store.loading && !store.movies.length" description="暂无欧美影片，请先扫描目录" />
    </div>
...
.date { font-size: 10px; color: #67c23a; }
.tags { margin-top: 4px; }
